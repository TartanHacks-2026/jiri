code_patterns.md: Engineering Standards for Jiri
...
if chunk.is_audio: # Non-blocking forward to Azure
...
return subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE) 3. Agentic & RAG Patterns
...
return AwesomeMCPDiscoveryHandler() 5. Error Handling & Safety
...
async def wrapper(*args, \*\*kwargs): # Trigger Siri/Agentuity approval UI
approved = await request_user_confirmation(func.**name**)
if not approved:
